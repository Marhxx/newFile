...
<div>
	<form id="editfm" method="post" enctype="multipart/form-data">
			<div class="fitem">
				<label>应用名称:</label> <input name="taiName" id="taiName"
					...
					data-options="prompt:'请输入应用名称'" value="${applicatonInfo.taiName }" /><span style="color: red">*</span>
			</div>
			<div class="fitem">
				<label>应用地址:</label> <input name="taiWebAddr" id="taiWebAddr"
					class="easyui-textbox" style="width: 171px;"
					data-options="prompt:'请输入应用地址'" value="${applicatonInfo.taiWebAddr}"/><span style="color: red">*</span>
			</div>
			<div class="fitem">
			...
			<div class="fitem">
				...
				<textarea name="taContent" id="taContent" cols="100" rows="8" style="width: 700px; height: 300px; visibility: hidden;">${applicatonInfo.taContent}</textarea>
			</div>
	...
	<script type="text/javascript">
		...
		function update(id) {
			...
			if(isEmpty('${applicatonInfo.taiIcon}') && isEmpty(filename)){
				$.messager.alert("提示信息", "请选择图标");
				return false;
			}
			$("#editfm").form("submit", {
				url : '${ctx}/admin/applicaton/update/'+id+'.htm?',
				onsubmit : function() {
					return $(this).form("validate");
				},
				success : function(result) {
					if (result == "1") {
						location.href="${ctx}/admin/applicaton/list.htm";
					} else {
