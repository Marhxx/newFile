    ...
    $.fn, {
        ...
        bitmapCutter: function(settings) {
            ...
            var ps = $.fn.extend({
                ...
                holderSize: { width: 252, height: 277 },
            ...
            $bcglobal.$label_pictures = ps.label_pictures
            $bcglobal.$label_preview = ps.label_preview
            $bcglobal.$btnLabel = ps.btnLabel
            ///<sammary>
            /// zoom interface
            ///</sammary>
            ...
            function irotate(angle) {
                ...
                //Use 'rel' but not 'src',  this helps to make sure that the 'src' attribute was not start with 'http://localhost:8888/...'(msie)
                  src = img.attr('rel');
                $.ajax({
                    url: 'http://localhost:8080/JqueryUpate/MyHtml.html',
                    dataType: 'json',
...
//                }
            ...
            var scissors = {
                ...
                zoomin: function() {
                    ...
                    var inv = Math.min($bcglobal.$zoomValue + ps.zoomStep, ps.zoomIn)*100+"px"
                    $(".child").css("left",inv)
                },
                zoomout: function() {
                    //window.console && console.log('zoom value: %s', zoomValue);
                    izoom.call(this, Math.max($bcglobal.$zoomValue - ps.zoomStep, ps.zoomOut));
                    var outv = Math.max($bcglobal.$zoomValue - ps.zoomStep, ps.zoomOut)*100+"px"
                    $(".child").css("left",outv)
                },
                original: function() {
                    izoom.call(this, 1, 1);
                    $(".child").css("left","82.5px")
                },
                clockwise: function() {
            ...
            var $cl = $('<div class="editlogo_boxleft"   onselectstart="return false;"></div>').appendTo(ps.renderTo);
            
            var $editlogo = $("<div />").addClass("editlogo_boxright").appendTo(ps.renderTo);
          
            var $cr = $('<div id="look" class="editlogo_boxrightbot_s" style="padding-left: 172px; padding-bottom: 5px;"> </div>');
	            $editlogo.append($cr);
	            $editlogo.append('<p class="pdlr">'+$bcglobal.$label_pictures+'</p><div class="spacer"></div><div class="spacer"></div><div class="spacer"></div>');
            
            var $editlogoLook = $('<div />').css("padding-left","4px");
            
            var $crc = $('#cut');
            var $crs = $('#look');
            	$editlogoLook.append($crs).append('<p>'+$bcglobal.$label_preview+'</p>');
            	$editlogo.append($editlogoLook);
            	$('<div class="clear"></div><div class="spacer"></div><div class="spacer"></div><div class="spacer"></div><div class="spacer"></div>').appendTo(ps.renderTo);
            var $btnp = $('<p/>').addClass("aligncenter");
            var $generate = $('<input type="button" id="cancelbtn" class="button_blue_2c" name="cancelbtn" value="'+$bcglobal.$btnLabel+'" style="margin-top:200px;margin-left: 50px;" />')	
            	$btnp.append($generate).appendTo(ps.renderTo);
            	
           
            //bitmap holder//p class="aligncenter"><input type="button" id="cancelbtn" class="button_blue_2c" name="cancelbtn" value="保存" /></p>
            var $holder = $('<div class="jquery-bitmapcutter-holder" />').css(ps.holderSize).appendTo($cl);
            
            
            var $opts = $('<div class="jquery-bitmapcutter-opts" >' +
            			'<table><tbody><tr><td id="Min"><a class="zoomout" onfocus="this.blur()" href="javascript:void(0)"  title="Zoom out">&nbsp;</a></td>'+
            			'<td><div id="bar"><div class="child " style="left: 82.5px; top: 0px;"> </div></div></td>'+
            			'<td id="Max"><a class="zoomin" onfocus="this.blur()" href="javascript:void(0)" title="Zoom in">&nbsp;</a></td>'+
            			'<td><a href="javascript:void(0)" onfocus="this.blur()" class="original">&nbsp</a></td>'+
            			'</tr></tbody></table></div>').insertAfter($holder);
            //options
//            var $opts = $('<div class="jquery-bitmapcutter-opts" >' +
...
//                                '</div>').insertAfter($holder);
            ...
            //image
            var $img = $('<img id="holderImg" alt="" rel="' + ps.src + '" />')
                                        .appendTo($holder);

            //thumbnail
            var $thumbimg = $('<img  alt="" rel="' + ps.src + '" />')
            										.appendTo(
												                $('<div class="jquery-bitmapcutter-thumbnail"  style="margin-left: 2px; margin-top: 3px;"/>')
												                    .css(ps.cutterSize)
												                        .appendTo($cr)
												                    );
            ...
            $img.loadBitmap(function(e) {
                ...
                $(".child").draggable({
                    ...
                    drag: function(e, ui) {
	                	...
	                	var $childLeftMum = $childLeft.substring(0,$childLeft.length-2);
	                	izoom($childLeftMum/100)
                ...
                $generate.click(function() {
                	...
                	var img = $bcglobal.$img.scaleBitmap()
                    var me = $(this);
...
//                    $cutter.fadeOut();
                   ...
                   // $processed.fadeIn();
                    ...
                    $.ajax({
                        ...
                        error: function(err) {
                            $(document.body).html(err.responseText);
...
//                                $cutter.fadeIn();
                            	//$info.show();
//                                $processed.fadeOut();
//                                $newimg.attr('src', json.src);
                                ps.onGenerated(json.src);
                                $("#chooseHeadImg").hide();
                                $("#met_logo").attr("src", json.src)
                                $("#tcHeadPhoto").attr("value", json.src)
                                $("#mmLogo").attr("value", json.src)
								$("#tc_head_photo").attr("src", json.src)
                            }
                            else {
                            	$.prompt(json.msg,{alertType:'msg'});
                               //alert(json.msg);
                            }
                        }
                    });
